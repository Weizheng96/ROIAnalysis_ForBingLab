...
cd('D:\Data\Kenichi\Box Sync\WZ and KI_collaboration 2021\04282021_vnc_iav-Gal4_10steps_7 sine tones_80dBpeak\MatData');

NumAnimal=4;

load("#3D#_#"+1+"_vnc.mat");
[d1,d2,d3]=size(dat);
Y=zeros(d1,d2,d3,NumAnimal);
for cnt=1:NumAnimal
    load("#3D#_#"+cnt+"_vnc.mat");
    Y(:,:,:,cnt)=imresize3(double(dat),[d1,d2,d3]);
end
...
options_rigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'d3',size(Y,3),'bin_width',200,'max_shift',15,'us_fac',50,'init_batch',200);
...
options_nonrigid = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),'d3',size(Y,3),'grid_size',[32,32,32],'mot_uf',1,'bin_width',200,'max_shift',15,'max_dev',3,'us_fac',50,'init_batch',5);
tic; [M2,shifts2,template2,options_nonrigid] = normcorre_batch(Y,options_nonrigid); toc
[M3,shifts2,template2,options_nonrigid] = normcorre(Y,options_nonrigid);
%% test, added by Wei
M_final = apply_shifts(Y(:,:,:,2),shifts2(2),options_nonrigid);

implay(mat2gray(template2))
for cnt=1:4
    tifwrite(mat2gray(M3(:,:,:,cnt)),convertStringsToChars("Reg_#"+cnt));
end
%% compute metrics
